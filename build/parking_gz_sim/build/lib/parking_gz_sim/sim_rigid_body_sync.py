...
class SimRigidBodySync(Node):
    def __init__(self):
        ...
        # 강체축 회전 속도 제한 — max_yaw_corr보다 작아야 개별 로봇 yaw가
        # 축 회전을 따라갈 수 있다 (크면 front/rear yaw가 벌어져 fail-safe 정지)
        self.declare_parameter('max_path_omega', 0.12)
        ...
        self.max_path_w = float(gp('max_path_omega').value)
        ...
        # False면 cmd_vel 발행을 완전히 중단 — 그리퍼 노드가 미세 정렬 중
        # 제어권을 가져갈 때 사용 (정지 명령도 내보내지 않음)
        self.enabled = True
        self.create_subscription(Bool, '/sync/enable', self.enable_cb, 10)
    ...
    def enable_cb(self, msg):
        if bool(msg.data) != self.enabled:
            ...
            self.get_logger().info(
                '주행 제어 재개' if self.enabled else '주행 제어 양도 (그립 시퀀스)')
    ...
    def control_loop(self):
        ...
        if hard_stop:
            ...
            # 완전 정지하면 오차가 스스로 줄지 않아 교착된다.
            # 중심 이동 없이 간격/yaw 동기 보정만 계속 수행해 복구.
            self.recover_sync(dist_err, f_axis_err, r_axis_err,
                              axis_x, axis_y, fth, rth)
            return
        ...
        # 경로 방향으로 강체축 정렬
        # holonomic 쌍이라 후진 주행이 가능하므로, 경로가 뒤쪽(>90°)이면
        # 180° 돌지 않고 축을 반대 방향으로 정렬한 채 후진한다.
        path_yaw = math.atan2(dy, dx)
        path_err = ang_norm(path_yaw - heading)
        if abs(path_err) > math.pi / 2:
            path_err = ang_norm(path_err + math.pi)
        omega = self.clamp(self.kp_path * path_err,
                           -self.max_path_w, self.max_path_w)
    ...
    def recover_sync(self, dist_err, f_axis_err, r_axis_err,
                     axis_x, axis_y, fth, rth):
        """오차 과대 정지 중 복구: 중심은 고정하고 간격/yaw만 보정."""
        sync_corr = self.clamp(self.kp_dist * dist_err,
                               -self.max_sync_corr, self.max_sync_corr)
        f_vx_w = -0.5 * sync_corr * axis_x
        f_vy_w = -0.5 * sync_corr * axis_y
        r_vx_w = 0.5 * sync_corr * axis_x
        r_vy_w = 0.5 * sync_corr * axis_y

        f_w = -self.clamp(self.kp_ysync * f_axis_err,
                          -self.max_yaw_corr, self.max_yaw_corr)
        r_w = -self.clamp(self.kp_ysync * r_axis_err,
                          -self.max_yaw_corr, self.max_yaw_corr)

        self.publish_cmd(self.pub_f, self.make_cmd(f_vx_w, f_vy_w, f_w, fth))
        self.publish_cmd(self.pub_r, self.make_cmd(r_vx_w, r_vy_w, r_w, rth))
